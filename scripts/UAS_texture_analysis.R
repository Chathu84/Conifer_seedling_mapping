...
wd = "D:/Carbon_dynamics/UAS-processing/Schoonover-9-6-2022"

setwd(wd)


file_list = dir("D:/Carbon_dynamics/UAS-processing/Schoonover-9-6-2022/data/Schoonover/09-06-2022", pattern = "*chm2.tif", full.names = FALSE, ignore.case = TRUE)
...
RGB_file = "schoonover-9-6-2022-ortho.tif"
...
for (i in 2:length(names_to_cloud)) #length(names_to_cloud)
{
  print(i)
  ...
  chm_img = raster(file.path(paste0("data/Schoonover/09-06-2022/",file_list[i])))
  ...
  all_layers <- file.path(wd,"testing", site_name,flight_datetime, paste0(names_to_cloud[i],"all_layers.tif"))
